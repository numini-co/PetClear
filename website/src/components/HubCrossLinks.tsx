...
import { getHubCrossLinks } from '../data/internalLinks.ts'
...
/** Visible contextual inbound/outbound strip. Pass `path` to drive links from the engine. */
export default function HubCrossLinks({
  ...
  const resolved =
    links ??
    (path
      ? getHubCrossLinks(path).map((l) => ({ to: l.to, label: l.label, note: l.note ?? '' }))
      : [])
