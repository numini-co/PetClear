# Reusable TS block — Airports + Pet-capable airlines

**Status:** PATTERN ONLY — do not Cursor-launch page tranche until all 50 dog+flag heroes are ready (owner gate).  
**Consume:** `countryAirAir.seed.ts` + `LOCKED_50_URLS.txt`  
**Match:** existing PetClear visual language (`rounded-[20px]`, `#4F5BD5`, FAQ schema later on full RoutePage).

Copy the component below into `website/src/components/RouteAirportsAirlinesBlock.tsx` when heroes are ready / Frontend tranche starts.

```tsx
import type { AirlineRow, AirportRow } from '../data/routes/countryAirAir.seed'

type Props = {
  countryName: string
  direction: 'inbound' | 'outbound'
  countryAirports: AirportRow[]
  uaeAirports?: AirportRow[]
  airlines: AirlineRow[]
}

const MODE_LABEL: Record<AirlineRow['mode'], string> = {
...
export default function RouteAirportsAirlinesBlock({
  ...
  return (
    <section className="bg-white section-padding">
      <div className="mx-auto max-w-[1200px] px-5 sm:px-6 lg:px-8 space-y-12">
        <div>
          <h2 className="text-[24px] sm:text-[30px] font-bold text-[#2A2A2A]">
            Main airports — {countryName} {direction === 'inbound' ? '→ UAE' : '← from Dubai'}
          </h2>
          <p className="mt-3 text-[#5A5A5A] text-base leading-relaxed max-w-3xl">
              ...
              <tbody>
                {countryAirports.map((a) => (
                  <tr key={a.iata} className="border-t border-[#E6E8F5]">
                    ...
                    <td className="px-4 py-3 text-[#5A5A5A]">{a.city}</td>
              ...
              <tbody>
                {airlines.map((al) => (
                  <tr key={al.name} className="border-t border-[#E6E8F5]">
                    ...
                    <td className="px-4 py-3 text-[#5A5A5A]">{al.seedNotes}</td>
                    <td className="px-4 py-3 text-[#5A5A5A]">
                      {al.verificationStatus === 'verified' && al.policyUrl ? (
                        <a className="text-[#4F5BD5] font-semibold" href={al.policyUrl} target="_blank" rel="noopener noreferrer">
                          Official policy
                        </a>
                      ) : (
...
## Regulatory chrome (inbound callout — pair with block)
- MOCCAE import permit: **30 days from issuance** (never 90).  
- Titer when required: sample **within 90 days before travel** (≥0.5 IU/ml) — not a post-draw wait.  
- No invented permit/release/airline fee numerals — WhatsApp quote only (`+971504782999`).

## Owner gate
Do **not** launch Cursor to wire all 50 pages until Visual Media has all 50 dog+flag heroes. Page body copy waits for SEO volume pass; this block + tables can be reviewed now.
